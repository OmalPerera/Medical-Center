
/*

<table class="table">
    <tr>
        <th>
            @Html.DisplayNameFor(model => model.col_two)
        </th>
        <th>
            @Html.DisplayNameFor(model => model.col_three)
        </th>
        <th>
            @Html.DisplayNameFor(model => model.col_four)
        </th>
        <th></th>
    </tr>

@foreach (var item in Model) {
    <tr>
        <td>
            @Html.DisplayFor(modelItem => item.col_two)
        </td>
        <td>
            @Html.DisplayFor(modelItem => item.col_three)
        </td>
        <td>
            @Html.DisplayFor(modelItem => item.col_four)
        </td>
        <td>
            @Html.ActionLink("Edit", "Edit", new { id=item.col_one }) |
            @Html.ActionLink("Details", "Details", new { id=item.col_one }) |
            @Html.ActionLink("Delete", "Delete", new { id=item.col_one })
        </td>
    </tr>
}

</table>

*/







/*

// students index


<table class="table">
    <tr>
        <th>
            @Html.DisplayNameFor(model => model.stu_id)
        </th>
        <th>
            @Html.DisplayNameFor(model => model.stu_first_name)
        </th>
        <th>
            @Html.DisplayNameFor(model => model.stu_last_name)
        </th>
        <th>
            @Html.DisplayNameFor(model => model.stu_dob)
        </th>
        <th></th>
    </tr>

@foreach (var item in Model) {
    <tr>
        <td>
            @Html.DisplayFor(modelItem => item.stu_id)
        </td>
        <td>
            @Html.DisplayFor(modelItem => item.stu_first_name)
        </td>
        <td>
            @Html.DisplayFor(modelItem => item.stu_last_name)
        </td>
        <td>
            @Html.DisplayFor(modelItem => item.stu_dob)
        </td>
        <td>
            @Html.ActionLink("Edit", "Edit", new { id=item.stu_reg_number }) |
            @Html.ActionLink("Details", "Details", new { id=item.stu_reg_number }) |
            @Html.ActionLink("Delete", "Delete", new { id=item.stu_reg_number })
        </td>
    </tr>
}

</table>
*/
